...
<h3>Editer utilisateur </H3>
  ...
  <br/>
  <button type="submit" id="btnAddUser"  @click="submitPage" class="btn btn-primary float-end">Submit</button>
</form>
...
export default {
  ...
  methods:{
    editSubmitForm(event) {
      ...
      //this.$router.push({ name: 'dashboard_user_update', params: {id: this.data.user.id} });

      // Effectuez ici toute logique nécessaire, par exemple, soumettez les données à un serveur via une requête AJAX.
      this.$emit('emitEditSubmitUser',this.formData,this.data.user.id);
